...
public function getArrayFromHandle(handle arg) returns any[] {
    ...
    foreach int i in 0 ... count-1 {
        ...
        } else if (element is handle && !(java:toString(element) is ())){
            returnArray[i] = element;
        } else {
            returnArray[i] = element;
        }
...
public function getHandleFromObjectArray(JObject[] arg, string className) returns handle|error {
...
public function getHandleFromArray(anydata[] arg, string className) returns handle|error {
    handle returnHandle = jarrays:newInstance(check java:getClass(className), arg.length());
    int count=0;
    while (count < arg.length()) {
